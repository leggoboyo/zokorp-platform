...
describe("architecture review worker route", () => {
  ...
  it("accepts bearer authorization on GET requests", async () => {
    process.env.ARCH_REVIEW_WORKER_SECRET = "worker-secret";

    const response = await GET(
      new Request("http://localhost/api/architecture-review/worker?limit=2", {
        method: "GET",
        headers: {
    ...
    expect(response.status).toBe(200);
    expect(drainArchitectureReviewQueueMock).toHaveBeenCalledWith({ limit: 2 });
  });
});
