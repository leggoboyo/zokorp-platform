...
- Diagnostic processing layer: free-tool submissions and paid validator parsing live in `app/api/submit-architecture-review/route.ts`, `app/api/submit-ai-decider/route.ts`, `app/api/submit-cloud-cost-leak-finder/route.ts`, `app/api/submit-landing-zone-readiness/route.ts`, and `app/api/tools/zokorp-validator/route.ts`.
...
| Free diagnostic submission routes | Browser POST with JSON or multipart | Browser -> Next.js -> DB / Email / Zoho | High-volume public inputs with lead creation and result delivery | `app/api/submit-architecture-review/route.ts`, `app/api/submit-ai-decider/route.ts`, `app/api/submit-cloud-cost-leak-finder/route.ts`, `app/api/submit-landing-zone-readiness/route.ts` |
| Paid validator upload route | Authenticated multipart POST | Browser -> Next.js -> Parser / DB | PDF/XLSX parsing plus entitlement consumption | `app/api/tools/zokorp-validator/route.ts`, `lib/security.ts`, `lib/validator.ts` |
