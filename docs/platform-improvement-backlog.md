...
- [DONE][AUTO] CRIT-017 Rewrite the public privacy page so it accurately covers account verification, lead capture, result delivery, marketing follow-up, and data retention. Done when: the privacy page matches actual system behavior and references preference controls.
  - Evidence: `app/privacy/page.tsx` now explains work-email account posture, account history retention, optional archival and follow-up storage, provider-sharing boundaries, and preference controls, with regression coverage in `tests/public-contact-pages.test.tsx`.
- [DONE][AUTO] CRIT-018 Rewrite the public terms/support/refund policy surfaces for public launch readiness. Done when: the public site clearly covers service scoping, refund posture, support expectations, and tool-output disclaimers.
  - Evidence: `app/terms/page.tsx`, `app/support/page.tsx`, and `app/refunds/page.tsx` now describe founder-led soft-launch support scope, estimate-first services, tool-output limitations, refund-review posture, and customer support paths, with regression coverage in `tests/public-contact-pages.test.tsx`.
...
- [DONE][AUTO] CRIT-028 Add health reporting for the scheduled architecture worker, follow-up sender, and Zoho sync jobs. Done when: the repo has a documented and inspectable way to detect stale scheduled automation.
  - Evidence: `lib/admin-operations.ts` now summarizes automation freshness and failure signals for the architecture queue worker, architecture follow-ups, retention sweep, Zoho lead sync, and estimate-companion sync, `app/admin/operations/page.tsx` surfaces the health view, `app/api/internal/cron/zoho-sync-estimate-companions/route.ts` now emits run/failure audit logs, and `tests/admin-operations.test.ts` plus `tests/internal-zoho-sync-estimate-companions-route.test.ts` cover the new behavior.
